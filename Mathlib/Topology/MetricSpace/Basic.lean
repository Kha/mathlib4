...
def UniformSpace.Core.ofDist {α : Type _} (dist : α → α → ℝ) (dist_self : ∀ x : α, dist x x = 0)
  ...
  .ofFun dist dist_self dist_comm $ fun ε ε0 => ⟨ε / 2, half_pos ε0, fun x y z h₁ h₂ =>
    ...
    _ = ε := add_halves _⟩
#align uniform_space.core_of_dist UniformSpace.Core.ofDist

/-- Construct a uniform structure from a distance function and metric space axioms -/
def uniformSpaceOfDist (dist : α → α → ℝ) (dist_self : ∀ x : α, dist x x = 0)
    (dist_comm : ∀ x y : α, dist x y = dist y x)
    (dist_triangle : ∀ x y z : α, dist x z ≤ dist x y + dist y z) : UniformSpace α :=
  .ofCore (.ofDist dist dist_self dist_comm dist_triangle)
...
def Bornology.ofDist {α : Type _} (dist : α → α → ℝ) (dist_comm : ∀ x y, dist x y = dist y x)
  ...
  Bornology.ofBoundedNE { s : Set α | ∃ C, ∀ ⦃x⦄, x ∈ s → ∀ ⦃y⦄, y ∈ s → dist x y ≤ C }
    ⟨0, fun x hx y => hx.elim⟩ (fun s ⟨c, hc⟩ t h => ⟨c, fun x hx y hy => hc (h hx) (h hy)⟩)
    (fun s ⟨Cs, hs⟩ ⟨x, hx⟩ t ⟨Ct, ht⟩ ⟨y, hy⟩ => by
...
def PseudoMetricSpace.ofDistTopology {α : Type _} [TopologicalSpace α] (dist : α → α → ℝ)
  ...
  { dist := dist
    ...
    toUniformSpace :=
      { toCore := .ofDist dist dist_self dist_comm dist_triangle
        isOpen_uniformity := fun s => (H s).trans <| forall₂_congr fun x _ => by
          rw [← mem_comap_prod_mk]
          exact ((UniformSpace.Core.hasBasis_ofFun (exists_gt (0 : ℝ))
            dist _ _ _).comap (Prod.mk x)).mem_iff.symm }
    uniformity_dist := rfl
    toBornology := Bornology.ofDist dist dist_comm dist_triangle
    cobounded_sets := rfl }
#align pseudo_metric_space.of_metrizable PseudoMetricSpace.ofDistTopology
...
theorem toUniformSpace_eq : ‹PseudoMetricSpace α›.toUniformSpace =
...
theorem uniformity_basis_dist :
  ...
  delta uniformity; simp only [toUniformSpace_eq, uniformSpaceOfDist, UniformSpace.ofCore]
  exact UniformSpace.Core.hasBasis_ofFun (exists_gt _) _ _ _ _
...
theorem NNReal.nndist_eq (a b : ℝ≥0) : nndist a b = max (a - b) (b - a) := by
  ...
  rw [← NNReal.coe_eq, ← dist_nndist, NNReal.dist_eq, tsub_eq_zero_of_le h,
    max_eq_left (zero_le _), ← NNReal.coe_sub h, abs_of_nonneg (a - b).coe_nonneg]
...
instance pseudoMetricSpacePi : PseudoMetricSpace (∀ b, π b) := by
  ...
  show @IsBounded _ inferInstance s ↔ @IsBounded _ PseudoMetricSpace.toBornology _
...
theorem Real.dist_le_of_mem_pi_Icc {x y x' y' : β → ℝ} (hx : x ∈ Icc x' y') (hy : y ∈ Icc x' y') :
  ...
  refine' (dist_pi_le_iff dist_nonneg).2 fun b =>
    (Real.dist_le_of_mem_uIcc _ _).trans (dist_le_pi_dist _ _ b) <;> refine' Icc_subset_uIcc _
...
instance (priority := 100) second_countable_of_proper [ProperSpace α] : SecondCountableTopology α :=
  ...
  · exact ⟨⟨fun n => closedBall x n, fun n => is_compact_closedBall _ _, Union_closedBall_nat _⟩⟩
...
theorem properSpace_of_compact_closedBall_of_le (R : ℝ)
  ...
  ⟨by
    ...
    by_cases hr : R ≤ r
    · exact h x r hr
    · have : closedBall x r = closedBall x R ∩ closedBall x r :=
        by
        symm
        apply inter_eq_self_of_subset_right
        exact closedBall_subset_closedBall (le_of_lt (not_le.1 hr))
      rw [this]
...
instance (priority := 100) complete_of_proper [ProperSpace α] : CompleteSpace α :=
  ⟨by
    ...
    rcases(isCompact_iff_totallyBounded_isComplete.1 (is_compact_closedBall x 1)).2 f hf
...
instance prod_properSpace {α : Type _} {β : Type _} [PseudoMetricSpace α] [PseudoMetricSpace β]
    ...
    apply (is_compact_closedBall x r).Prod (is_compact_closedBall y r)
...
theorem exists_pos_lt_subset_ball (hr : 0 < r) (hs : IsClosed s) (h : s ⊆ ball x r) :
  ...
  have : IsCompact s :=
    isCompact_of_isClosed_subset (is_compact_closedBall x r) hs
      (subset.trans h ball_subset_closedBall)
  ...
  exact ⟨r', ⟨dist_nonneg.trans_lt hyr', hrr'⟩, subset.trans hy <| closedBall_subset_ball hyr'⟩
...
theorem Bounded.subset_ball_lt (h : Bounded s) (a : ℝ) (c : α) : ∃ r, a < r ∧ s ⊆ closedBall c r :=
  ...
  exact subset.trans hr (closedBall_subset_closedBall (le_max_left _ _))
...
theorem Bounded.union (hs : Bounded s) (ht : Bounded t) : Bounded (s ∪ t) := by
  ...
  exact
    ⟨max Cs Ct,
      union_subset (subset.trans hCs <| closedBall_subset_closedBall <| le_max_left _ _)
        (subset.trans hCt <| closedBall_subset_closedBall <| le_max_right _ _)⟩
...
protected theorem Bounded.prod [PseudoMetricSpace β] {s : Set α} {t : Set β} (hs : Bounded s)
  ...
  suffices : s ×ˢ t ⊆ closedBall x (max rs rt)
  exact bounded_closedBall.mono this
  rw [← @Prod.mk.eta _ _ x, ← closedBall_prod_same]
  exact
    prod_mono (hrs.trans <| closedBall_subset_closedBall <| le_max_left _ _)
      (hrt.trans <| closedBall_subset_closedBall <| le_max_right _ _)
...
theorem isCompact_of_isClosed_bounded [ProperSpace α] (hc : IsClosed s) (hb : Bounded s) :
  ...
  · rcases hb.subset_ball x with ⟨r, hr⟩
    exact isCompact_of_isClosed_subset (is_compact_closedBall x r) hc hr
...
theorem exists_local_min_mem_ball [ProperSpace α] [TopologicalSpace β]
  ...
  exact
    (is_compact_closedBall a r).exists_local_min_mem_open ball_subset_closedBall hf hz hf1
      is_open_ball
...
theorem comap_dist_right_atTop_le_cocompact (x : α) :
  ...
  exact ⟨Ioi r, Ioi_mem_at_top r, fun y hy hys => (mem_closedBall.1 <| hr hys).not_lt hy⟩
